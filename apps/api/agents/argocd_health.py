...
async def _list_applications() -> List[Dict[str, Any]]:
    """Fetch all ArgoCD Application statuses via the MCP server."""
    from agents.argocd_agent.tools import search_applications
    try:
        raw = await search_applications(query="", limit=200)
        # search_applications returns a formatted string; try to parse app names.
        return _parse_app_names(raw)
    except Exception as e:  # noqa: BLE001
        logger.warning(f"[ARGOCD] search_applications failed: {e}")
        return []
...
def _parse_app_names(raw: str) -> List[Dict[str, Any]]:
    ...
    for line in (raw or "").splitlines():
        line = line.strip()
        # Try to find lines that reference an application name.
        for token in line.replace("|", " ").split():
            if token and ("http" not in token and len(token) > 3 and token not in seen):
                # Heuristic: skip obvious non-names
...
async def _get_app_status(app_name: str) -> Optional[Dict[str, Any]]:
    """Get one ArgoCD application's status (health + sync) via MCP."""
...
async def check_and_trigger(application: str = "", dry_run: bool = False) -> Dict[str, Any]:
    ...
    if application:
        ...
        names = await _list_applications()
