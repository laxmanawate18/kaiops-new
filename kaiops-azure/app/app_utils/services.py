...
def get_session_service():
    ...
    if agent_engine_id := os.environ.get("GOOGLE_CLOUD_AGENT_ENGINE_ID"):
        ...
        # INFRA only. The engine session service must live in the agent-engine
        # region (us-central1). GOOGLE_CLOUD_LOCATION is pinned to "global" for
        # the MODEL endpoint and must never leak here (a "global" session
        # service would 404). Guard against model-endpoint aliases.
        def _infra_loc() -> str:
            loc = os.environ.get("GOOGLE_CLOUD_AGENT_ENGINE_LOCATION") or os.environ.get(
