"""Wait for the new identity-only governed engines to appear, verify NO gateway config,
and test they SERVE (:query). Writes to monitor_identonly.log. Run detached."""
import subprocess, json, urllib.request, urllib.error, time, os

LOG = r"f:\Personal\AI-Project\kaiops_latest\monitor_identonly.log"
    ...
    for i in range(50):
        ...
        try:
            ...
            if new:
                ...
                spec = e.get("spec",{})
                gw = "agentGatewayConfig" in str(spec.get("deploymentSpec",{}))
                ident = spec.get("identityType")
                cm = len(spec.get("classMethods",[]) or [])
                log(f"[{i}] NEW engine {eid}: ident={ident} gw={gw} cm={cm}")
                if cm >= 13:
                    # query it: async_create_session
                    qurl = f"https://{LOC}-aiplatform.googleapis.com/v1/projects/{PROJ}/locations/{LOC}/reasoningEngines/{eid}:query"
                    payload = {"class_method":"async_create_session","input":{"user_id":"u_identonly"}}
                    ...
                    try:
                        ...
                        log(f"[{i}] QUERY HTTP {qe.code}: {qe.read().decode()[:250]}")
                        return
                # if cm<13 (still deploying), keep waiting
                else:
                    log(f"[{i}] engine {eid} still deploying (cm={cm})")
            # else no new engine yet
